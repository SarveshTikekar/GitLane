...
class CommitGraphScreen extends StatelessWidget {
  const CommitGraphScreen({super.key, required this.commits});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Git Commit Graph')),
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          '${commit.id.substring(0, 7)}  |  ${_formatTimestamp(commit.timestamp)}',
...
List<_RowGraphState> _buildGraphRows(List<CommitNode> commits) {
  ...
  for (final commit in commits) {
    ...
    final bottomLanes = {...activeLanes}..remove(commit.lane)..addAll(parentLanes);
...
class _CommitGraphPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final targetLane in row.mergeToLanes) {
      ...
      canvas.drawLine(Offset(nodeX, centerY), Offset(targetX, size.height), edgePaint);
    }
